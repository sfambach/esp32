...
struct {
  ...
  displayType(SELECT_L_PIN, DISPLAY_DC, TFT_MOSI, TFT_SCLK, DISPLAY_RESET), SELECT_L_PIN, {NOBLINK},
...
void setup(void) {
  ...
  for (e = 0; e < NUM_EYES; e++) {
    ...
    eye[e].display.invertDisplay(false);
...
void drawEye( // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  digitalWrite(DISPLAY_DC, HIGH);                     // Data mode
   //eye[e].display.writePixels((uint16_t *)&pBurst[0], 128*128, (screenY == (128 - 1)), true);
   eye[e].display.drawRGBBitmap(0,0,(uint16_t *)&pBurst[0],128,128);
...
//  for(int y = 0 ; y < 128; y++){
//    for(int x = 0 ; x < 128; x++){
//        eye[e].display.writePixel(x,y,pBurst[i]);
//        i++;
//    }
//  }
//
